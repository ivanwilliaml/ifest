...
md("""
# Penyisihan IFEST 2026 DAC — V9: + Learned Substitution Patterns
...
## What V11 adds, and what it discarded after measuring

The V11 plan proposed exploiting near-duplicate articles and pair memory. **Measured first,
per the plan's own ceiling-analysis requirement — and the hypothesis is false:**

| neighbour article similarity | rows | P(label=0) |
|---|---|---|
| exact same body (existing rule) | - | **0.977** |
| sim >= 0.70 | 124 | 0.040 |
| sim >= 0.85 | 25 | 0.000 |

The rule does not merely weaken on near-duplicates, it **inverts** — such rows are *more*
likely positive than the 6% cold-start baseline. The reason is structural: tampered
headlines are attached to the **exact same body**, never to a merely similar one. A
near-duplicate is a different article carrying its own true headline. Near-duplicate
retrieval and pair memory are therefore dropped.

What survives is the plan's **learned substitution patterns**. Mining real 1<->1 token
swaps from same-body mixed-label pairs yields a concentrated tamper-target vocabulary
(`Jabar` 17x, `Bandung` 15x, `Malaysia` 12x, `Anies` 7x). The resulting feature —
*a known tamper-target entity present in the title but absent from the body* — fires on
2.7% of cold-start rows at **P(label=0)=0.425, a 7.0x lift**, far sharper than the generic
entity-missing signal (~0.19). Mining is done **per fold from that fold's training rows
only**, so no label information crosses the CV boundary.
""")
...
# Rule ablation (measured, n=871 held-out rows):
...
#              the pipeline that could be read as "merekonstruksi label", and it
#              buys almost nothing because 99.8% of those rows are label 1 and the
#              model already answers 1 there.
...
def build_features(df):
    ...
    for T,C,ch,lsa in zip(df['T'],df['C'],df['ch'],df['lsa_cos']):
        tw=toks(T); cw=toks(C)
        # --- salience: WHERE in the body a title entity first appears, and how
        # often. 42% of cold negatives have every title entity present, so
        # presence alone is blind to them; a tampered entity tends to be
        # peripheral (late, singleton) while a true headline names the lead.
...
print(pd.concat([Ftr[['n_title_ent_missing','swap_signature','sal_maxfirst','sal_out_of_lead','sal_singleton','num_mismatch']],
...
print("  most frequent targets:", sorted(_T_all)[:15])
""")
...
md("## 11. Blend + threshold under the test regime mixture")
...
with Timer("blend_threshold"):
    ...
    # Keep every non-cold row, not just rule-decided ones: with rule C and
    # B_no_pos off, those rows are answered by the MODEL at inference, so the
    # simulation has to score them with the model too or it overstates the result.
    rule_rows=[(r,Y_VA[i],RULE_VA[i],oof[rva[i]]) for i,r in enumerate(REG_VA) if r!='A_cold']
    rdf=pd.DataFrame(rule_rows,columns=['regime','y','pred','oof'])
    ...
    # pandas coerces the None entries of 'pred' to NaN, so test for null, not None
    _rp_defer=pd.isna(rp['pred']).values if len(rp) else np.array([],dtype=bool)
    _rp_rule=(np.nan_to_num(rp['pred'].values.astype(float),nan=0).astype(int)
              if len(rp) else np.array([],dtype=int))
    _rp_oof=rp['oof'].values.astype(float) if len(rp) else np.array([])
    _rp_y=rp['y'].values if len(rp) else np.array([])
    print(f"  simulation: {int(_rp_defer.sum())} of {len(_rp_defer)} non-cold rows "
          f"now answered by the MODEL (rule C / B_no_pos disabled)")
    def _mix_pred(t):
        return np.where(_rp_defer,(_rp_oof>=t).astype(int),_rp_rule)
    rows=[]
    for t in CONFIG['threshold_grid']:
        cp=(cold_oof>=t).astype(int)
        if CONFIG['USE_RULES'] and len(rp):
            ya=np.concatenate([cold_y,_rp_y]); pa=np.concatenate([cp,_mix_pred(t)])
        else:
            ya,pa=cold_y,cp
    ...
    # rules-only reference: model stubbed as all-ones everywhere it still decides
    _base_rule=np.where(_rp_defer,1,_rp_rule) if len(rp) else np.array([],dtype=int)
    base_pred=np.concatenate([np.ones(len(cold_y),int),_base_rule]) if len(rp) else np.ones(len(cold_y),int)
    base_y=np.concatenate([cold_y,_rp_y]) if len(rp) else cold_y
...
T=time.time()-RUN_T0
print("="*58)
print("V11 — compliant, rule C and B_no_pos removed")
...
with open('ifest2026_dac_v11.ipynb','w',encoding='utf-8') as f: nbf.write(nb,f)
